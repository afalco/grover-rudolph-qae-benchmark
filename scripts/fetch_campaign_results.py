#!/usr/bin/env python3
"""Fetch and analyze IBM Quantum campaign results."""
...
def latest_submitted_campaign() -> Path:
    ...
    if not candidates:
        raise SystemExit(
            "No submitted campaign found. Pass --campaign-run path/to/submitted_campaign.json."
        )
    return candidates[-1]
...
def get_counts_from_result(result) -> dict[str, int]:
    """Extract counts from a SamplerV2 result as robustly as possible."""
    pub_result = result[0]
    data = getattr(pub_result, "data", None)
    if data is None:
        raise ValueError("Result has no data field.")

    candidate_names = ["c", "meas", "measurements"]
    ...
    # DataBin-like objects often expose their fields through keys/items.
    try:
...
def ancilla_success_count(counts: dict[str, int]) -> int:
    """Count shots where the ancilla classical bit is 1.

    The calibration circuits measure q[0],q[1],q[2] into c[0],c[1],c[2].
    Qiskit count strings are displayed as c2c1c0, so the ancilla bit c[2] is
...
def mle_estimate(observations: list[dict[str, Any]]) -> dict[str, Any] | None:
    ...
    # Coarse grid followed by ternary search around the best grid point.
    grid_size = 20001
    lo = 1e-8
    hi = 1 - 1e-8
    ...
    a_hat = (left + right) / 2
    a_exact = observations[0].get("a_exact")
    if a_exact is None:
        a_exact = observations[0].get("quadrature_value")
    return {
        "case_id": observations[0].get("case_id"),
        "ks": [int(obs["k"]) for obs in observations],
        "a_hat": a_hat,
        "a_exact": a_exact,
        "abs_error": abs(a_hat - float(a_exact)) if a_exact is not None else None,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fetch IBM Quantum campaign results.")
    parser.add_argument(
        "--campaign-run",
        default=None,
...
def main() -> int:
    ...
    for record in records:
        ...
        if is_done_status(status):
            try:
                ...
                m_ancilla = ancilla_success_count(counts)
                p_hat = m_ancilla / shots if shots else None
                ...
                row.update(
                    {
                        ...
                        "m_ancilla": m_ancilla,
                        "p_hat": p_hat,
                        "p_abs_dev": abs(p_hat - expected_p)
                ...
                result_path = results_dir / f"result_{record['case_id']}_k{record['k']}_{job_id}.json"
    ...
    completed_rows = [row for row in rows if row.get("p_hat") is not None]
    estimates = []
    for case_id in sorted({row["case_id"] for row in completed_rows}):
        case_rows = sorted(
            [row for row in completed_rows if row["case_id"] == case_id],
    ...
    summary_json = run_dir / "fetched_campaign_results.json"
    summary_csv = run_dir / "fetched_campaign_results.csv"
    ...
    if estimates:
        ...
        for estimate in estimates:
            error = estimate.get("abs_error")
            error_text = f"{error:.6f}" if error is not None else "n/a"
            print(
                f"  {estimate['case_id']} K={estimate['ks']}: "
                f"a_hat={estimate['a_hat']:.8f} "
                f"error={error_text}"
            )
    return 0
