#!/usr/bin/env python3
"""Prepare or submit Grover-Rudolph distribution-measurement campaigns."""
...
DEFAULT_CAMPAIGN = PROJECT_ROOT / "experiments" / "phase3_gr_distribution_ibm_fez.json"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Prepare or submit a Grover-Rudolph distribution campaign."
    )
    ...
    parser.add_argument("--yes", action="store_true", help="Skip confirmation with --submit.")
...
def main() -> int:
    ...
    implementation = str(args.implementation or campaign.get("implementation", "ucry"))
    ...
    selected_cases = campaign.get("selected_cases", [])
    if not selected_cases:
        raise SystemExit("Campaign has no selected_cases.")
    ...
    for case in selected_cases:
        ...
        qasm_path = circuits_dir / f"{spec.case_id}_transpiled.qasm"
        ...
        record = {
            ...
            "campaign_type": "gr_distribution_measurement",
    ...
    for item in prepared:
        ...
        print(
            f"  {record['case_id']}: n={record['num_qubits']} "
            f"grid={record['grid_points']} depth={tm['depth']} "
            f"twoq={tm['two_qubit_gate_count']} "
    ...
    for item in prepared:
        ...
        print(f"Submitting {record['case_id']} ...", end=" ", flush=True)
        try:
            ...
            if submitted_records:
                ...
                submitted_path.write_text(
                    json.dumps(submitted_records, indent=2, sort_keys=True),
                    encoding="utf-8",
                )
                print(f"Partial submitted metadata: {submitted_path}")
        ...
        pending_path = metadata_dir / f"pending_{record['case_id']}_{job_id}.json"
